...
def do_monthly_user_site_hits_transformation(spark, dataframe, ds):
    ...
    SELECT
           month_start,
           SUM(COALESCE(try_element_at(hit_array, 1), 0)) as num_hits_first_day,
           SUM(COALESCE(try_element_at(hit_array, 2), 0)) AS num_hits_second_day,
           SUM(COALESCE(try_element_at(hit_array, 3), 0)) as num_hits_third_day
